...
function loadNameKanji(): KanjiDetail[] {
  // 仮のデータ（実際のデータ構造に合わせて調整が必要）
  // ここでは isName フラグを持つ漢字をフィルタ
  const dictionary = loadKanjiDictionary();
  return dictionary.filter((k) => (k as any).isName === true);
}

export default async function NameKanjiPage() {
  const nameKanji = loadNameKanji();

  // 画数順にソート
  nameKanji.sort((a, b) => a.strokes - b.strokes);
  ...
  return (
    <main className="flex flex-col items-center gap-8 w-full max-w-4xl mx-auto">
      ...
      <header className="text-center">
        <h1 className="text-4xl font-bold mb-2">👤 人名漢字一覧</h1>
        <p className="text-muted-foreground max-w-xl">
          人名用漢字（姓・名で使われやすい）の一覧です。表外漢字の中でも人名に使われる漢字の書き順・読み方・意味を学習できます。
        </p>
        <p className="text-sm text-muted-foreground mt-2">{nameKanji.length}字</p>
      </header>
              ...
              {nameKanji.map((k) => (
                ...
                >
                  ...
                  <div className="mt-1 text-xs text-muted-foreground text-center space-y-0.5">
                    ...
                    <span className="block px-1 py-0.5 bg-purple-100 text-purple-700 rounded text-[10px]">
                      人名
                    </span>
                  </div>
                </Link>
